...
def sll(rd, rs1, rs2):
    global PC
    rs1_value = unsigned_decimal(register_values[rs1][2:])  # Convert binary to int
    rs2_value = unsigned_decimal(register_values[rs2][2:]) & 0b11111  # Extract 5 LSBs
    rd_value = (rs1_value << rs2_value) & 0xFFFFFFFF  # Ensure 32-bit result

    register_values[rd] = '0b' + format(rd_value, '032b')  # Store as binary string
    PC = '0b' + format(unsigned_decimal(PC[2:]) + 4, '032b')  # Increment PC by 4
...
def J(i):
    ...
    # print_register_states(output_file)
...
register_values = {
    ...
    'sp': '0b00000000000000000000000101111100',
    'gp': '0b00000000000000000000000000000000', 'tp': '0b00000000000000000000000000000000',
    ...
    'a2': '0b00000000000000000000000000000000', 'a3': '0b00000000000000000000000000000000',
    ...
    's2': '0b00000000000000000000000000000000', 's3': '0b00000000000000000000000000000000',
    ...
    's8': '0b00000000000000000000000000000000', 's9': '0b00000000000000000000000000000000',
...
dict_registers = {
    '00000': 'zero', '00001': 'ra', '00010': 'sp', '00011': 'gp', '00100': 'tp', '00101': 't0', '00110': 't1',
    '00111': 't2', '01000': 's0',
    '01001': 's1', '01010': 'a0', '01011': 'a1', '01100': 'a2', '01101': 'a3', '01110': 'a4', '01111': 'a5',
    '10000': 'a6', '10001': 'a7',
    '10010': 's2', '10011': 's3', '10100': 's4', '10101': 's5', '10110': 's6', '10111': 's7', '11000': 's8',
    '11001': 's9', '11010': 's10',
...
while (data != halt):
    ...
    if data[-7:] == R_OPCODE:
        ...
        J(data)
    register_values['zero'] = "0b00000000000000000000000000000000" # reset the zero value!
    #
    a = ((unsigned_decimal(PC[2:]) // 4))
    data = input_lines[a]
    if data != halt:
    ...
    output_element = PC
    print(unsigned_decimal(output_element[2:]), end = " ")
    # print(signed_decimal(output_element[2:]), end = " ")
    for reg in register_values.keys():
        # print(signed_decimal(register_values[reg][2:]), end = " ")
        output_element += ' ' + register_values[reg]
        print(unsigned_decimal(register_values[reg][2:]), end = " ")
    print()
...
with open(output_file, 'w') as f:
    ...
    for keys, values in list(memory_values.items())[:32]:
        print(f"{keys}:{values}", end="")
        f.write(keys + ':' + values)
        f.write('\n')
        print(f"{keys}: {unsigned_decimal(values[2:])}")
